...
$(document).on('turbolinks:load', function () {
  ...
  function drawchart(data) {
    var thickness_h = d3.sum(data, d => parseFloat(d.thickness));
    ...
    d3.select('.stratChart').append('g').attr('class', 'axis axis--y').call(
    ...
    var tooltip = d3.select('html').append('div').attr('class', 'tool').style(
      'background-color', 'white').style('border', '1px solid black').style(
      'padding', '12px').style('border-radius', '8px').style('position',
      'absolute').style('z-index', '10').style('visibility', 'hidden').style(
      'font-size', '12px');
    // Tooltip action
    d3.selectAll('.bar, .bar-overlay, .unconformity').on('mouseover', d =>
      tooltip.style('visibility', 'visible').html(
        `Lithology: ${d.name}</br>Formation: ${d.formation}</br>Geologic Age: ${d.timescale.interval_name}</br>Upper Contact: ${d.contact.name}</br>Thickness (m): ${d.thickness}</br>Lithology Pattern: ` +

        (d.lithology.name3 !== "" ?
          d.lithology.name + ' / ' + d.lithology.name2 + ' / ' + d.lithology
          .name3 :
          d.lithology.name2 !== '' ?
          (d.lithology.name + ' / ' + d.lithology.name2) :
          d.lithology.name)
      )
    ).on('mousemove', () => tooltip.style('top', (event.pageY - 120) +
      'px').style('left', event.pageX + 15 + 'px')).on('mouseout', () =>
    ...
    legendRectSize *= 1.5;
    legendSpacing *= 2;
    var legend = stratChart.selectAll('.legend-lithology')
      .data(filteredData)
      .enter()
      .append('g')
