...
before_action :authenticate_user!
  ...
  def data
    ...
    render(json: @layers, except: [:created_at, :updated_at], include: [{strat_column: {except: [:created_at, :updated_at]}},{lithology: {except: [:created_at, :updated_at]}}, {timescale: {only: [:interval_name, :color, :abbrev]}}, {contact: {except: [:created_at, :updated_at]}}, {fossils: {except: [:created_at, :updated_at]}}] )
  ...
  def lithologies_data
    @lithologies = Lithology.where('name ILIKE :search OR name2 ILIKE :search OR name3 ILIKE :search OR url LIKE :search', search: "%#{params[:term]}%")
